...
int main() {
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 2: {
                string customWord;
                int lengthCustomWord;
                while (true) {
                    cout << endl << "YOUR CUSTOM WORD (max 30 letters): " << endl;
                    cin >> customWord;
                    lengthCustomWord = customWord.length();
                    if (lengthCustomWord > 30) {
                        cout << "SORRY, TOO LONG!";
                    }
                    else {
                        for (int i = 0; i < lengthCustomWord; i++) {
                            if (customWord[i] < 65 || customWord[i] > 90 && customWord[i] < 97 || customWord[i] > 122) {
                                cout << "SORRY, LETTERS ONLY, NO SYMBOLS" << endl;
                                noDigits = false;
                                break;
                            }
                        }
